...
object BinaryTrees {
  ...
  /**
    ...
    */
  implicit val treeFFunctor: Functor[TreeF] =
    new Functor[TreeF] {
      override def map[A, B](fa: TreeF[A])(f: A => B): TreeF[B] =
        fa match {
          case BranchF(label, left, right) =>
            BranchF(label, f(left), f(right))
          case LeafF(label) =>
            LeafF(label)
          case EmptyF() => EmptyF()
        }
  ...
  /**
    ...
    */
  implicit val treeBirecursive: Birecursive.Aux[Tree, TreeF] =
    Birecursive.fromAlgebraIso(treeAlg, treeCoalg)
  ...
  def toList: Algebra[TreeF, List[Int]] = {
    ...
    case EmptyF()     => List.empty[Int]
  }

  val testTree: Recursive.AllOps[Tree, TreeF] =
    Branch(12, Branch(10, Leaf(1), Empty()), Leaf(15))
  ...
  def fromList: Coalgebra[TreeF, List[Int]] = {
    ...
    case x :: Nil => LeafF(x)
    case x :: xs =>
      BranchF(x, xs.filter(_ < x), xs.filter(_ > x))
  }

  /**
    * I wonder what this mystery function does…
    */
  def mystery(input: List[Int]): List[Int] =
    input.hylo(toList, fromList)
...
object Test extends App {
  ...
  println(mystery(List(5, 1, 20, 10)))
}
